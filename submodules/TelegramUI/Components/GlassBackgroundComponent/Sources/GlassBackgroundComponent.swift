...
public final class GlassBackgroundContainerView: UIView {
    ...
    private let fallbackEffectView: UIVisualEffectView?
    ...
    public override init(frame: CGRect) {
        var hasNativeGlass = false
        if #available(iOS 26.0, *), !GlassBackgroundView.useCustomGlassImpl {
            if let effectClass = NSClassFromString("UIGlassContainerEffect") as AnyObject as? NSObjectProtocol {
                ...
                self.legacyView = nil
                self.fallbackEffectView = nil
                self.fallbackTintView = nil
                hasNativeGlass = true
            }
        }
        ...
        if !hasNativeGlass {
            ...
            self.fallbackEffectView = effectView
            
            let tintView = UIView()
            tintView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            tintView.isUserInteractionEnabled = false
            self.fallbackTintView = tintView
        ...
        if let nativeParamsView = self.nativeParamsView {
            ...
            if let fallbackEffectView = self.fallbackEffectView {
                self.addSubview(fallbackEffectView)
            }
            if let fallbackTintView = self.fallbackTintView {
                self.addSubview(fallbackTintView)
            }
            self.addSubview(legacyView)
        }
    }
    ...
    override public func didAddSubview(_ subview: UIView) {
        ...
        if subview !== self.nativeParamsView && subview !== self.legacyView && subview !== self.fallbackEffectView && subview !== self.fallbackTintView {
            assertionFailure()
        }
    }
    ...
    public func update(size: CGSize, isDark: Bool, transition: ComponentTransition) {
        if let nativeParamsView = self.nativeParamsView, let nativeView = self.nativeView {
            ...
            transition.setFrame(view: legacyView, frame: bounds)
            
            if let fallbackEffectView = self.fallbackEffectView {
                transition.setFrame(view: fallbackEffectView, frame: bounds)
                let targetStyle: UIBlurEffect.Style
                if #available(iOS 13.0, *) {
                    targetStyle = isDark ? .systemMaterialDark : .systemMaterialLight
                } else {
                    targetStyle = isDark ? .dark : .light
                }
                let targetEffect = UIBlurEffect(style: targetStyle)
                if fallbackEffectView.effect != targetEffect {
                    fallbackEffectView.effect = targetEffect
                }
            }
            if let fallbackTintView = self.fallbackTintView {
                transition.setFrame(view: fallbackTintView, frame: bounds)
                let tintColor = isDark
                    ? UIColor(white: 0.08, alpha: 0.40)
                    : UIColor(white: 1.0, alpha: 0.50)
                transition.setBackgroundColor(view: fallbackTintView, color: tintColor)
            }
        }
    }
