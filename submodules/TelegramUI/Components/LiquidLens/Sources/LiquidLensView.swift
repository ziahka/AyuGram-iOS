...
private final class RestingBackgroundView: UIVisualEffectView {
    ...
    init() {
        ...
        for subview in self.subviews {
            let desc = subview.description
            if desc.contains("VisualEffectSubview") && !desc.contains("Backdrop") {
    ...
    func update(isDark: Bool) {
        ...
        var backdropSublayer: CALayer?
        if let sublayers = self.layer.sublayers {
            for layer in sublayers {
                if layer.filters != nil && !(layer.filters?.isEmpty ?? true) {
                    backdropSublayer = layer
                    break
                }
            }
            if backdropSublayer == nil, !sublayers.isEmpty {
                backdropSublayer = sublayers[0]
            }
        }
        
        if let sublayer = backdropSublayer, let _ = sublayer.filters {
...
public final class LiquidLensView: UIView {
    ...
    private static let useSystemLiquidLens = false
    ...
    public init(kind: Kind) {
        ...
        if Self.useSystemLiquidLens, let viewClass = NSClassFromString("_UILiquidLensView") as AnyObject as? NSObjectProtocol {
