...
AudioDeviceModuleIOS::AudioDeviceModuleIOS(bool bypass_voice_processing, bool disable_recording, bool enableSystemMute, int numChannels)
  ...
  int32_t AudioDeviceModuleIOS::Init() {
    ...
    if (pendingDarkgramVoiceLabConfiguration_) {
      audio_device_->setDarkgramVoiceLab(
        std::get<0>(*pendingDarkgramVoiceLabConfiguration_),
        std::get<1>(*pendingDarkgramVoiceLabConfiguration_),
        std::get<2>(*pendingDarkgramVoiceLabConfiguration_)
      );
      pendingDarkgramVoiceLabConfiguration_.reset();
    }
    return 0;
  }
  ...
  void AudioDeviceModuleIOS::setDarkgramVoiceLab(bool enabled, int32_t pitchCents, int32_t outputGainPercent) {
    if (audio_device_) {
      audio_device_->setDarkgramVoiceLab(enabled, pitchCents, outputGainPercent);
    } else {
      pendingDarkgramVoiceLabConfiguration_ = std::make_tuple(enabled, pitchCents, outputGainPercent);
    }
  }
