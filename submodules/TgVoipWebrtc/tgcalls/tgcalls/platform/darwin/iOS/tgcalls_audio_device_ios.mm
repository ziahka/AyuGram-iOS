...
void AudioDeviceIOS::setDarkgramVoiceLab(bool enabled, int32_t pitchCents, int32_t outputGainPercent) {
    RTC_DCHECK(thread_);
    thread_->PostTask(SafeTask(safety_, [this, enabled, pitchCents, outputGainPercent] {
        _darkgramRealtimeVoiceProcessor.SetConfiguration(enabled, pitchCents, outputGainPercent);
    }));
}
...
OSStatus AudioDeviceIOS::OnDeliverRecordedData(AudioUnitRenderActionFlags* flags,
  ...
  if (record_audio_buffer_.size() > 0) {
    _darkgramRealtimeVoiceProcessor.Process(record_audio_buffer_.data(), record_audio_buffer_.size());
  }
